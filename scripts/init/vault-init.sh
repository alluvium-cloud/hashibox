...
echo "--- [conad-server-1] Unseal Leader
...
echo "${init}" | tee bootstrap-vault.log &> >/dev/null
...
VAULT_ADDR=http://conad-server-1.alluvium.cloud:8200 vault operator unseal ${unsealKey} 
...
for ip in "${IP_SERVERS[@]}"; do
    echo "--- [$ip] Raft Join to conad-server-1:8200"
